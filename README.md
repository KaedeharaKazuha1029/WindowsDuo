...
本分支（`surface`）提供 Surface Pro 等 Windows 二合一设备的内置姿态传感器实现。
...
| 版本 | 平台 | 开合角度来源 | 硬件要求 |
|---|---|---|---|
| **Windows + Surface 内置传感器** | Windows | 设备内置 Inclinometer | 支持 Windows Sensor Platform 的二合一设备（如 Surface Pro） |
...
```
surface/                 Surface Pro 等二合一设备内置传感器版
...
### Windows + Surface 内置传感器

1. 确认设备支持 Windows Sensor Platform（Surface Pro 等二合一设备）。
2. 安装依赖：
   ```powershell
   ...
   ```
3. 运行 Surface 版本：
   - `surface/run_overlay.bat` — 内置姿态传感器驱动（键盘 `r` 切换手动/自动）
   - `surface/run_manual.bat` — 纯键盘手动
4. 键盘操作（先点一下控制台窗口获得焦点）：`↑/↓` 浓度 ±3%，`←` 清零，`→` 拉满，`r` 切换控制方，`Esc` 退出。

当前角度约定：合上 `-90°`，打开 `0°`，中间姿态按 Inclinometer 的绝对 `pitch` 线性映射。程序优先读取 `Inclinometer` 的绝对姿态；陀螺仪仅显示实时角速度，不把累计变化量当作绝对开合角。
...
### Surface 版本 (`surface/config.json`)

| 参数 | 含义 |
|---|---|
| `inclinometer_axis` | 使用 `pitch` 或 `roll` |
| `inclinometer_closed` / `inclinometer_open` | 设备合上 / 打开时的实际传感器读数 |
...
### ESP32 / macOS 版本 (`win/config.json`, `mac/config.json`)
